...
function renderTodo(doc){
    ...
    cross.addEventListener('click', (e) => {
        ...
        db.collection("/users/" + user + "/profile/apps/todo list").doc(id).delete();
    })
...
db.collection("/users/" + user + "/profile/apps/todo list").onSnapshot(snapshot => {
...
function renderGoal(doc){
    ...
    cross.addEventListener('click', (e) => {
        e.stopPropagation();
        let id = e.target.parentElement.getAttribute('data-id');
        db.collection("/users/" + user + "/profile/apps/goal list").doc(id).delete();
    })
...
db.collection("/users/" + user + "/profile/apps/goal list").onSnapshot(snapshot => {
    let changes = snapshot.docChanges();
    changes.forEach(change => {
        if(change.type == 'added'){
